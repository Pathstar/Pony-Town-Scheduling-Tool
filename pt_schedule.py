...
if __name__ == '__main__':
    ...
    date_string = "2025-12-16 15:32:18"
    # game_time_string = "23:50"
    game_time_string = "01:10"
    ptm = GameTimeManager(date_string, game_time_string)

    def time_test():
        print(f"2025-12-16 15:32:17 的游戏时间 {ptm.get_game_time_by_datetime(datetime.strptime("2025-12-16 15:32:17", "%Y-%m-%d %H:%M:%S"))}")
        print(f"现在的游戏时间 {ptm.get_game_time()}")
        print(f"距离下次游戏时间变化: {ptm.get_seconds_to_next_game_minute()}")
        # 减少-0.001以上的睡眠才会无法越过下一次游戏时间更新
        time.sleep(ptm.get_seconds_to_next_game_minute())
        # 距离下次: 1.9991683959960938
        print(f"距离下次游戏时间变化: {ptm.get_seconds_to_next_game_minute()}")
        print(ptm.get_game_time())
        # for i in range(1000):
        #     time.sleep(1e-6)
        #     print(ptm.get_game_time())
    ...
    time.sleep(100)
    # day_sec = 24 * 60 * 60
    # print(day_sec/ ptm.game_day_seconds)
    # 30
    ...
    # beijing_time_precise = datetime.now(beijing_tz)
    #
    # print(f"精确北京时间: {beijing_time_precise.strftime('%Y-%m-%d %H:%M:%S %Z')}")
    # print(f"本地时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")

#
#     print('当前游戏时间:', gtm.get_game_time())
#
#     def my_callback(hh, mm):
#         print('游戏时间触发:', f"{hh:02d}:{mm:02d}")
#
#     scheduler = GameTimeScheduler(gtm)
#     scheduler.add_callback(9, 1, my_callback)  # 在游戏09:01触发
#     scheduler.add_callback(9, 2, my_callback)  # 在游戏09:02触发
#
#     scheduler.start()
#
#     # 主线程持续2分钟，演示功能
